...
* Use this value to run out of ROM
	ORG		$00082000			* past the vectors in a real system
...
* Subsequent times, send READ <filename> n 1
        ...
        LEA      LAB_READN,A0 * Send READ command string
...
ENDLN   MOVE.B          #0,load_filename(A2)            * Add terminating null to filename
        ...
        LEA             LAB_WRITE,A0                * Send WRITE command string
        BSR             PRINTSTRING2                    * Print null terminated string
...
LAB_sizok
	...
	LEA		UART_ReadChar,a1		* get byte from input device vector
	MOVE.l	a1,(a0)+			* set vector

	MOVE.w	d0,(a0)+			* V_OUTP JMP opcode
	LEA		UART_WriteChar,a1		* send byte to output device vector
	MOVE.l	a1,(a0)+			* set vector
...
*##LAB_GMEM
	...
	LEA		des_sk,a4			* set descriptor stack start
	...
	LEA		LAB_SMSG,a0		* point to start message
	BSR		LAB_18C3			* print null terminated string from memory

	LEA		LAB_RSED,a0		* get pointer to value
	BSR		LAB_UFAC			* unpack memory (a0) into FAC1

	LEA		LAB_1274,a0		* get warm start vector
...
LAB_XERR
	...
	BSR		LAB_CRLF			* print CR/LF
	LEA		LAB_BAER,a1		* start of error message pointer table
	MOVE.w	(a1,d7.w),d7		* get error message offset
	LEA		(a1,d7.w),a0		* get error message address
	BSR		LAB_18C3			* print null terminated string from memory
	LEA		LAB_EMSG,a0		* point to " Error" message
LAB_1269
	BSR		LAB_18C3			* print null terminated string from memory
...
LAB_1274
	LEA		LAB_RMSG,a0		* point to "Ready" message
...
LAB_1491
	LEA		des_sk,a4		* reset descriptor stack pointer

	MOVE.l	(sp)+,d0			* pull return address
	LEA		$86000,sp		* set stack to RAM start + 1k, flush stack
...
LAB_152E
	...
	LEA		LAB_KEYT,a2		* get keyword table address
	...
	LEA		TAB_STAR,a2		* get keyword table address
...
LAB_1602
	...
	ADD.w		d0,d0				* *2
	LEA		LAB_CTBL,a0		* get vector table base address
...
LAB_164F
	...
	LEA		LAB_BMSG,a0		* point to "Break"
...
*	BSR.s		LAB_1212			* check room on stack for A bytes
	...
	PEA		LAB_15C2		* set return address
...
LAB_16B0
	BSR		LAB_GBYT			* scan memory
	PEA		LAB_15C2		* return to interpreter inner loop after GOTO n
...
LoopAlways
	...
	LEA		LAB_15C2,a0		* get return address
...
LAB_1904
	...
	BRA		LAB_TMER			* do type mismatch error, then warm start
							...
							* mode was INPUT
LAB_1913
	LEA		LAB_REDO,a0		* point to redo message
	BSR		LAB_18C3			* print null terminated string from memory
...
LAB_19C2
	...
	PEA		LAB_195B		* set return address
...
LAB_1A0E
	...
	RTS
							* user typed too much
LAB_1A1B
	LEA		LAB_IMSG,a0		* point to extra ignored message
...
LAB_1B43						*
	LEA		LAB_OPPT,a0		* point to operator vector table
...
LAB_1B13
	...
	LEA		LAB_OPPT,a0		* set pointer to operator table
...
LAB_1B3C
	LEA		LAB_OPPT,a0		* point to function vector table
	CMP.w		(a0,d1.w),d0		* compare with this opperator precedence
...
LAB_SCGB
	PEA		LAB_GTBY		* return address is to get byte parameter
...
* value at the top of this file to some non zero value
 ...
 ifne	novar
	...
	LEA.l		LAB_1D96,a0		* else return a null descriptor/pointer
...
* for functions that returned strings
	...
	LEA		LAB_FTBL,a0		* pointer to functions vector table
	MOVE.w	(a0,d0.w),d1		* get function vector offset
	PEA		(a0,d1.w)			* push function vector

	LEA		LAB_FTPP,a0		* pointer to functions preprocess vector table
...
LAB_1D5E
	...
	LEA		LAB_FVAR,a2		* get the address of the create if doesn't
...
LAB_201E
	...
	LEA		des_sk_e,a1		* get string stack pointer max+1
...
* done arrays and variables, now just the descriptor stack to do
	...
	LEA		des_sk,a2		* set end pointer to end of stack
...
LAB_224D
	PEA		LAB_1ADB		* continue evaluation after concatenate
...
LAB_LENS
	PEA		LAB_AYFC		* set return address to convert d0 to signed
							* longword in FAC1
...
LAB_CALL
	PEA		LAB_GBYT		* put return address on stack
...
* a0 = table pointer
	...
	MOVEQ		#0,d6				* z = 0
	MOVE.l	#1<<30,d1			* y = 1
	LEA		TAB_HTHET,a0		* get pointer to hyperbolic tangent table
...
LAB_2953
	LEA		LAB_LMSG,a0		* point to " in line " message
...
LAB_295E
	LEA		Bin2dec,a1		* get table address
...
LAB_29FB
	LEA		LAB_2A9A,a0		* get base of table
...
* d5 = loop count
	...
	LEA		TAB_HTHET,a0		* get pointer to hyperbolic arctan table
...
LAB_SIN
	...
	LEA		TAB_SNCO,a0		* get pointer to constants table
...
LAB_ATLE
	...
	LEA		TAB_ATNC,a0		* get pointer to arctan table
	MOVEQ		#0,d6				* Z = 0
	MOVE.l	#1<<30,d1			* y = 1
...
LAB_U002
	...
	ADD.w		d0,d1				* * 3
	ADD.w		d1,d1				* * 6
	LEA		LAB_P_10,a0		* get the rounding table base
...
LAB_2Y04
	...
	LEA		LAB_P_10,a0		* point to table
